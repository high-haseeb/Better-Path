...
let outputNode_vo, outputNode_ambience;
...
function triggerVO_sensibility() {
    if(audioContextStarted) {
        path = vo_sensibility[getRandomInt(0,vo_sensibility.length-1)];
...
function triggerVO_temptation() {
    if(audioContextStarted) {
        path = vo_temptation[getRandomInt(0,vo_temptation.length-1)];
        console.log("Loading: " + path);
        setupSample(path).then((response) => {
            sample = response;
            if(voPlayers) {
